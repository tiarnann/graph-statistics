...
bn<-function(dat,method=c("mple.triad","mple.dyad","mple.edge","mtle"),param.seed=NULL,param.fixed=NULL,optim.method="BFGS",optim.control=list(),epsilon=1e-5){
  ...
  if(match.arg(method)%in%c("mple.edge","mple.dyad")){  #Use dyad census stats
    ...
    stats<-matrix(.C("bn_dyadstats_R",as.integer(dat),as.double(n), stats=as.double(stats),PACKAGE="CycleAnalysisLib")$stats,ncol=4)
    ...
    stats<-matrix(.C("bn_triadstats_R",as.integer(dat),as.double(n), stats=as.double(stats),PACKAGE="CycleAnalysisLib")$stats,nrow=n,ncol=n)
  ...
  out$triads.pred<-.C("bn_ptriad_R", as.double(out$pi),as.double(out$sigma),as.double(out$rho), as.double(out$d),pt=as.double(rep(0,16)),PACKAGE="CycleAnalysisLib")$pt
...
bn.nlpl.dyad<-function(p,stats,fixed=rep(NA,4),...){
  ...
  lpl<-0
  lpl<-.C("bn_lpl_dyad_R",as.double(stats),as.double(NROW(stats)), as.double(p[1]),as.double(p[2]),as.double(p[3]),as.double(p[4]), lpl=as.double(lpl),PACKAGE="CycleAnalysisLib")$lpl
  -lpl
}
...
bn.nlpl.triad<-function(p,dat,stats,fixed=rep(NA,4),...){
  ...
  #Calculate the pseudolikelihood
  lpl<-0
  lpl<-.C("bn_lpl_triad_R",as.integer(dat),as.double(stats), as.double(NROW(stats)),as.double(p[1]),as.double(p[2]),as.double(p[3]), as.double(p[4]), lpl=as.double(lpl),PACKAGE="CycleAnalysisLib")$lpl
  -lpl
}
...
bn.nltl<-function(p,stats,fixed=rep(NA,4),...){
  ...
  pt<-rep(0,16)
  triprob<-.C("bn_ptriad_R", as.double(p[1]),as.double(p[2]),as.double(p[3]), as.double(p[4]),pt=as.double(pt),PACKAGE="CycleAnalysisLib")$pt
...
brokerage<-function(g,cl){
  ...
  br<-matrix(0,N,5)
  br<-matrix(.C("brokerage_R",as.double(g),as.integer(N),as.integer(m), as.integer(icl), brok=as.double(br),PACKAGE="CycleAnalysisLib",NAOK=TRUE)$brok,N,5)
...
lnam<-function(y,x=NULL,W1=NULL,W2=NULL,theta.seed=NULL,null.model=c("meanstd","mean","std","none"),method="BFGS",control=list(),tol=1e-10){
   ...
   agg<-function(a,w){
     ...
     matrix(.C("aggarray3d_R",as.double(a),as.double(w),mat=mat,as.integer(m), as.integer(n),PACKAGE="CycleAnalysisLib",NAOK=TRUE)$mat,n,n)
   }
