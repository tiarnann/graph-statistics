...
logMean<-function(x){
  if(length(x)==0)
    ...
    .C("logadd_R",as.double(x),as.integer(length(x)),lsum=as.double(0), NAOK=TRUE,PACKAGE="CycleAnalysisLib")$lsum-log(length(x))
}
...
logSub<-function(x,y){
  if(length(x)!=length(y))
    ...
    .C("logsub_R",as.double(x),as.double(y),as.integer(length(x)), ldiff=as.double(rep(0,length(x))),NAOK=TRUE,PACKAGE="CycleAnalysisLib")$ldiff
}
...
logSum<-function(x){
  if(length(x)==0)
    ...
    .C("logadd_R",as.double(x),as.integer(length(x)),lsum=as.double(0), NAOK=TRUE,PACKAGE="CycleAnalysisLib")$lsum
}
