...
connectedness<-function(dat,g=NULL){
   ...
   if(n<=1)
     ...
     con<-.C("connectedness_R",as.double(g),as.integer(n),as.integer(m), con=as.double(0.0),PACKAGE="CycleAnalysisLib",NAOK=TRUE)$con
   #Return the result
   con
}
...
dyad.census<-function(dat,g=NULL){
   ...
   intcalc<-function(m){
     ...
     if(NROW(m)>0){
       dc<-.C("dyadcode_R",as.double(m),as.integer(n),as.integer(NROW(m)), dc=as.double(rep(0,NROW(m))),PACKAGE="CycleAnalysisLib",NAOK=TRUE)$dc
    ...
    recip<-sapply(dat,function(z){             #Compute the measure
      ...
      if(NROW(z)>0){                             #Process edges
        ...
        if(gv==0)                                  #If var 0, treat as corr 1
          return(1)
        dc<-.C("dyadcode_R",as.double(z),as.integer(n),as.integer(NROW(z)), dc=as.double(rep(0,NROW(z))),PACKAGE="CycleAnalysisLib",NAOK=TRUE)$dc
        odc<-order(dc)
...
gtrans<-function(dat,g=NULL,diag=FALSE,mode="digraph",measure=c("weak","strong","weakcensus","strongcensus","rank","correlation"),use.adjacency=TRUE){
  ...
  if(use.adjacency){  #Use adjacency matrix - much faster for n<1000 or dense
    ...
    gt<-.C("transitivity_R",as.double(dat),as.integer(attr(dat,"n")), as.integer(NROW(dat)),gt=as.double(c(0,0)),as.integer(meas),as.integer(1),NAOK=TRUE,PACKAGE="CycleAnalysisLib")$gt
   ...
   intlub<-function(g){
      ...
      for(i in 1:max(cd$membership)){   #Walk through the components
         ...
         if(length(vi)>2){  #Components must be of size 3
           ...
           viol<-as.double(0)
           viol<-.C("lubness_con_R",as.double(g[vi,vi]), as.double(length(vi)),as.integer(r[vi,vi]),viol=viol,PACKAGE="CycleAnalysisLib")$viol
           nolub<-nolub+viol
...
triad.census<-function(dat,g=NULL,mode=c("digraph","graph")){
   ...
   for(i in 1:length(dat)){
     ...
     if(n>2)
       tcm[i,]<-.C("triad_census_R",as.double(dat[[i]]),n,m,tcv=tcv,gm, as.integer(1),PACKAGE="CycleAnalysisLib", NAOK=TRUE)$tcv
   }
   colnames(tcm)<-tc
   rownames(tcm)<-rnam
   #Return the result
...
triad.classify<-function(dat,g=1,tri=c(1,2,3),mode=c("digraph","graph")){
   ...
   tt<-as.integer(0)
   gm<-as.integer(switch(match.arg(mode),graph=0,digraph=1))
   tt<-.C("triad_classify_R",as.integer(d),tt=tt,gm,PACKAGE="CycleAnalysisLib")$tt
   tc[tt+1]
}
